...
'''
Todo: 
- verify fix the multiple triggering (log creation?)
- detect non-responsive arduino after a while
- parallel threading to handle multiple sockets???
- verify traffic limits of this, dunno if that is too much traffic potentially

- ~~dump triggered from gamemaster?~~ 
not needed gamemaster can restart arduinos directly which will trigger the dump
- optional backup of the current states in case of crash


- make sure no permission issue on RPi exist when it comes to USB access
- check log generation and folders there could be an issue in there
...
# is a candidate for a refactoring
